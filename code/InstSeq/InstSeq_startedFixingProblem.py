...
class Inst_Seq(threading.Thread):
    ...
    def callback_giapi2(self, action_id, seq_cmd, activity, config):
        print(f'################### seq_cmd: {seq_cmd} action_id: {action_id} Completedddddddddd saving in A' )
        #giapi.CommandUtil.postCompletionInfo(action_id, giapi.HandlerResponse.create(giapi.HandlerResponse.COMPLETED))       
        #print(f'sleeping1')
        #ti.sleep(5)
        a = instDummy.DataResponse(giapi.HandlerResponse.COMPLETED, "")
        #print(f'sleeping2')
        #ti.sleep(20)
        print(f':::::::::::::::::::: \n \n')
        return a
        ...
        try:
            ...
            # SequenceCommand.INIT    
                ...
                if activity == giapi.command.Activity.PRESET:
                    ...
                    for idx in range(DCS_CNT):
                        self.dcs_ready[idx] = False
                    print(f'Calling to initialized22222222222')
                    self.initialize2()
                    print(f'sent initialized333')
    ...
    def connect_to_server_ex(self):
        print(f'self.ics_ip_addr: {self.ics_ip_addr}')
    ...
    def publish_to_queue(self, msg):
        print (f'publissing_to_queue: producer: {self.producer == None}, seding: {msg}')
    ...
    def dcs_data_processing_SVC(self, param):
        ...
        if param[0] == CMD_INITIALIZE1:
            ...
            #if self.cur_action_id == 0: return  // if the detector is initialized, the correct thing is update the value
    ...
    def dcs_data_processing_HK(self, param, idx):
        print(f'dcs_data_processing_HK: param:{param} idx: {idx} self.cur_action_id: {self.cur_action_id} self.dcs_ready[idx]: {self.dcs_ready[idx]}') 
        if param[0] == CMD_INITIALIZE1:
            ...
            if self.rebooting[H]:
                ...
                if self.dcs_ready[SVC] and self.dcs_ready[H] and self.dcs_ready[K]:
                    self.response_complete(bool(int(param[1])))
                    #print(f'Should call the completed')
